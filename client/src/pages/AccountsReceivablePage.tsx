...
import { TrendingUp, Plus, DollarSign, RefreshCw, ChevronLeft, ChevronRight, CheckCircle, Filter, X, Save } from 'lucide-react';
import { useDialog } from '../contexts/DialogContext';
import { usePageTitle } from '../hooks/usePageTitle';
import { accountsReceivableService, type AccountsReceivable, type AccountsReceivableStats } from '../services/accountsReceivable.service';
import { partnerService } from '../services/partner.service';
import { chartOfAccountsService } from '../services/chartOfAccounts.service';

const AccountsReceivablePage = () => {
    usePageTitle('Contas a Receber');
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const [receivablesRes, statsRes, partnersRes, accountsRes] = await Promise.all([
                accountsReceivableService.getAll(params),
            ...
            setTotalItems(receivablesRes.meta.total);
            setTotalPages(receivablesRes.meta.totalPages);
            setStats(statsRes);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            dialog.success({
                title: 'Conta Criada!',
                message: 'A conta a receber foi criada com sucesso.',
            });
            ...
            dialog.error({
                ...
                message: 'Não foi possível criar a conta a receber.',
            });
        }
    };

    const handleOpenPaymentModal = (receivable: AccountsReceivable) => {
        setSelectedReceivable(receivable);
        setPaymentData({
            paidAmount: Number(receivable.amount),
            paymentMethod: 'PIX',
        });
    ...
    const handleRegisterPayment = async (e: React.FormEvent) => {
        ...
        try {
            await accountsReceivableService.pay(selectedReceivable.id, paymentData);
            dialog.success({
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-slate-500">Carregando contas a receber...</div>
            </div>
        );
    }
        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-600 mt-1">Gestão de recebíveis</p>
                </div>
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {receivables.map((receivable) => (
                                            <tr key={receivable.id} className="border-b hover:bg-slate-50">
                                                <td className="p-4">{receivable.description}</td>
                                                <td className="p-4">{receivable.partner.name}</td>
                                                <td className="p-4">R$ {Number(receivable.amount).toFixed(2)}</td>
                                                <td className="p-4">{new Date(receivable.dueDate).toLocaleDateString('pt-BR')}</td>
                                                <td className="p-4">{getStatusBadge(receivable.status)}</td>
                                                <td className="p-4">
            ...
            {/* Create Modal */}
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} icon={<TrendingUp size={24} />} title="Nova Conta a Receber">
                ...
                <form onSubmit={handleRegisterPayment} className="space-y-4">
                    ...
                    {selectedReceivable && (
                        <div className="bg-slate-50 p-4 rounded">
                            ...
                            <p className="font-medium">R$ {Number(selectedReceivable.amount).toFixed(2)}</p>
                        </div>
                    )}
